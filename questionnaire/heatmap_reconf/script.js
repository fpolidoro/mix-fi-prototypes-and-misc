...
Promise.all(
    ...
    d3.csv("../../data/steps_10.csv").then(function (data) {
        ...
        const groups = dataArray.slice().sort((a,b) => a.getMonth() > b.getMonth()).map(d => d.getMonth())
        const myVars = dataArray.slice().sort((a,b) => {
        ...
        var width = groups.length > 1 ? (groups[groups.length-1] - groups[0])*gridsize : gridsize
        width *= 3
        ...
        const stepCount = function(d) {
            let count = data.filter(dd => {
                ...
                return ddate.getMonth() === date.getMonth() && ddate.getWeekDay() === date.getWeekDay()
            })
