...
Promise.all(
  ...
  function zoom(id, area){
    ...
    .on("start", e => {
      ...
      el.style.backgroundPositionX = `${lastPosY}px`
    ...
    .on("zoom", e => {
      if(e.transform.k - startK > 0 && lastPosY <= -MIN_1){
        ...
        el.style.backgroundPositionX = `${lastPosY += MIN_1}px`
    ...
    .on("end", e => {
      ...
      if(e.transform.k - startK > 0 && lastPosY <= -MIN_1){
        console.log(`Zoom in ${e.transform.k}`)
        el.style.backgroundPositionX = `${lastPosY = -LAST_1}px`
      }else if(e.transform.k - startK < 0 && lastPosY >= -MAX_1){
        console.log(`Zoom out ${e.transform.k}`)
        el.style.backgroundPositionX = `${lastPosY = 0}px`
      }
