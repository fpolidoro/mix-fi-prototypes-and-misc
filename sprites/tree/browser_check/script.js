...
Promise.all(
  ...
  function zoom(id, area){
    ...
    .on("start", e => {
      ...
      el.style.backgroundPositionX = `${lastPosX}px`
      el.style.backgroundPositionY = `413px`
      startK = e.transform.k
    })
    .on("zoom", e => {
      if(e.transform.k - startK > 0 && lastPosX <= -MIN_1){
        //console.log(`Zoom in ${e.transform.k}`)
        el.style.backgroundPositionX = `${lastPosX += MIN_1}px`
      }else if(e.transform.k - startK < 0 && lastPosX >= -MAX_1){
    ...
    .on("end", e => {
      ...
      if(e.transform.k - startK > 0 && lastPosX <= -MIN_1){
        console.log(`Zoom in ${e.transform.k}`)
        el.style.backgroundPositionX = `${lastPosX = -LAST_1}px`
      }else if(e.transform.k - startK < 0 && lastPosX >= -MAX_1){
        console.log(`Zoom out ${e.transform.k}`)
        el.style.backgroundPositionX = `${lastPosX = 0}px`
      }
