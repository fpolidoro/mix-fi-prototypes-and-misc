...
$("#answer519483X4X10").hide()
$("#answer519483X4X10").val(0)
...
ia2Click$.pipe(
  ...
  if(i === 1){  //hide interactive area
    ...
    $("#answer519483X4X10").val(actionDone.reduce((accumulator, currentValue) => accumulator + currentValue, 0));
  }
})
...
window.rxjs.zip(
  ...
  window.rxjs.mergeMap(([start1, start2]) => {
    ...
    return window.rxjs.fromEvent(anim, 'touchmove').pipe(
      window.rxjs.tap(mm => mm.preventDefault()),   //prevent scroll
...
semaphoreEnd$.pipe(
  ...
  if(i === 1){  //hide interactive area
    ...
    lastPosX$.next(pos);
    $("#answer519483X4X10").val(actionDone.reduce((accumulator, currentValue) => accumulator + currentValue, 0)).trigger('keyup');
  }
});
